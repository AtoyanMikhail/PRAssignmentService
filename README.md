...
- Запустит swaggerUI на https://localhost:8081
...
- **Go**: Выбран за производительность, type safety и отличную поддержку concurrency
- **PostgreSQL**: Соответствие ACID, поддержка JSON и надежные инструменты
- **Gin**: Легковесный, быстрый HTTP фреймворк с хорошей экосистемой middleware
- **sqlc**: Генерация type-safe SQL кода для валидации запросов на этапе компиляции
- **Docker**: Консистентная среда разработки и развертывания
